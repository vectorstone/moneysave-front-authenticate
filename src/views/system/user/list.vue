<template>
  <div class="app-container">
    ...
    <div class="tools-div" style="margin-top: 10px">
      <!--添加的按钮-->
      <el-button type="success" icon="el-icon-plus" size="mini" @click="add" >添 加</el-button> <!--:disabled="$hasBP('bnt.sysUser.add')  === false"-->
      <!-- 批量删除的按钮 -->
    ...
    <el-table
      ...
      <el-table-column label="操作" align="center" fixed="right">
        <template slot-scope="scope">
          <el-button type="primary" icon="el-icon-edit" size="mini" @click="edit(scope.row.id)" title="修改" :disabled="$hasBP('bnt.sysUser.update')  === false"/><!-- :disabled="$hasBP('bnt.sysUser.update')  === false" -->
          <el-button type="danger" icon="el-icon-delete" size="mini" @click="removeDataById(scope.row.id)"
                     title="删除" :disabled="$hasBP('bnt.sysUser.remove')  === false"/><!-- :disabled="$hasBP('bnt.sysUser.remove')  === false" -->
          <el-button type="warning" icon="el-icon-baseball" size="mini" @click="showAssignRole(scope.row)" title="分配角色" :disabled="$hasBP('bnt.sysUser.assignRole')  === false"/><!-- :disabled="$hasBP('bnt.sysUser.assignRole')  === false" -->
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="添加/修改" :visible.sync="dialogVisible" width="40%">
      <el-form ref="dataForm" :model="sysUser" label-width="100px" size="small" style="padding-right: 40px;">
        <el-form-item label="用户名" prop="username" >
          <el-input v-model="sysUser.username" @blur="checkUsername"/><!--@blur="checkUsername" TODO 检查用户名是否唯一的先不要加-->
        </el-form-item>
...
<script>
import api from '@/api/system/accountUser'
// import sysRoleApi from '@/api/system/accountSysRole'
...
export default {
  data() {
    return {
      ...
      limit: 4, // current page show data size
      list: [], // sysUser List
  ...
  methods: {
    //用户名必须唯一,新增一个失去焦点的时候检查用户名是否唯一的方法
    checkUsername(){
      //不用接收响应,有异常会直接弹出提示
      console.log('检查用户名')
      api.checkUsername(this.sysUser.username)
    },
    //1.下面的是和用户角色修改相关的方法
    // 1.1展示用户的角色修改的列表
    showAssignRole(row){
      ...
      sysRoleApi.getAssign(row.id).then(res=>{
    ...
    fetchData(pageNum = 1) {
      ...
      api.getPageList(this.page, this.limit, this.searchObj).then(res => {
        ...
        this.page = Number(res.data.items.current)
        this.limit = Number(res.data.items.size)
    ...
    saveOrUpdate() {
      if (this.sysUser.id) {
        ...
        api.editUser(this.sysUser).then(res => {
          //提示成功的信息
